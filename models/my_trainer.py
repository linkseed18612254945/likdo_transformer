...
class MyPredictionOutput(NamedTuple):
...
class Trainer:
    ...
    def train(self, model_path: Optional[str] = None, test_func=None, test_data=None, test_label_names=None):
        """
        ...
        for epoch in train_iterator:
            ...
            # my epoch evaluate
            y_true, y_predict, mres, all_text_vectors, all_label_vectors, lv = test_func(test_data, test_label_names)
            print(mres)
            print(classification_report(y_true, y_predict, target_names=test_label_names))
    ...
    def prediction_loop(
        ...
        for inputs in tqdm(dataloader, desc=description):
            loss, logits, labels, temp_info = self.prediction_step(model, inputs, prediction_loss_only)
            if loss is not None:
            ...
            if len(extra_info) > 0:
                for k, v in temp_info.items():
                    if isinstance(v, torch.Tensor):
                        extra_info[k] = torch.cat((extra_info[k], v), dim=0)
                    else:
                        extra_info[k].extend(v)
            else:
        ...
        return MyPredictionOutput(predictions=preds, label_ids=label_ids, metrics=metrics, extra_info=extra_info)
    ...
    def prediction_step(
    ...
    ) -> Tuple[Optional[float], Optional[torch.Tensor], Optional[torch.Tensor], Optional[Dict]]:
        """
        ...
        with torch.no_grad():
            ...
            if has_labels:
                ...
                loss = None
                logits = outputs[0]
            extra_info = {k: v for k, v in outputs.items() if k not in ['loss', 'logits']}
            if self.args.past_index >= 0:
        ...
        return (loss, logits.detach(), labels, extra_info)
